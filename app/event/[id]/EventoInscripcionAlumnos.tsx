...
const EventoInscripcionAlumnos = ({ event, user }) => {
  ...
  const validateExcelData = (data) => {
    ...
    data.forEach((row, index) => {
      ...
      if (row["DNI Pareja"] && !/^\d{8}$/.test(row["DNI Pareja"].toString())) {
  ...
  const checkDNIsInFirebase = async () => {
    ...
    try {
      ...
      const checkPromises = uniqueDNIs.map(async (dni) => {
        try {
          const q = query(usersCollection, where("dni", "==", dni.toString()));
          const snapshot = await getDocs(q);
  
          if (!snapshot.empty) {
            results.validUsers.push(dni);
          } else {
            results.invalidUsers.push(dni);
          }
        } catch (err) {
