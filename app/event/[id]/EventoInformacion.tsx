...
const EventoInformacion: React.FC<EventoInformacionProps> = ({ 
  ...
  // Use the props if provided, otherwise fallback to false
  const individualWebEnabled = isIndividualWebEnabled !== undefined ? isIndividualWebEnabled : false;
  const grupalCSVEnabled = isGrupalCSVEnabled !== undefined ? isGrupalCSVEnabled : false;
            ...
            <div className="mt-4 flex flex-col gap-3">
              {!isOrganizer && (
                <button
                  ...
                  className={`w-full py-2 px-4 rounded-lg text-sm md:text-base font-medium transition-all duration-300 hover:shadow-md active:scale-95 
                ...
                <button
                  ...
                  className={`w-full py-2 px-4 rounded-lg text-sm md:text-base font-medium transition-all duration-300 hover:shadow-md active:scale-95
                    ${grupalCSVEnabled 
                      ...
                      : "bg-gray-300 text-gray-500 cursor-not-allowed"}`}
                >
                  {grupalCSVEnabled ? "Inscribir alumnos" : "Inscripción grupal no disponible"}
                </button>
              )}
            </div>
          </div>
