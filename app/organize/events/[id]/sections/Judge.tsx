...
import { Users, Play, Pause, SkipForward, Grid3X3, List, Search, Star, Clock, Award, Download, Settings, ChevronRight, ChevronLeft, Trophy, FileText, Filter, Timer, AlertCircle, BarChart3, Medal, Eye, Target, TrendingUp, Save } from 'lucide-react';
...
const Judge: React.FC<LiveProps> = ({ event }) => {
  ...
  const [timerLimit, setTimerLimit] = useState(300);
  ...
  const participantsWithUserData = participants.map((participant) => {
    const relatedUsers = users.filter(u => participant.usersId.includes(u.id));
    return { ...participant, users: relatedUsers };
  });

  const filteredParticipants = participantsWithUserData.filter(p => {
    const userNames = p.users.map(u => `${u.firstName} ${u.lastName}`.toLowerCase());
    const matchesSearch = userNames.some(name => name.includes(searchTerm.toLowerCase()));
    const matchesCategory = categoryFilter === 'Todas' || p.category === categoryFilter;
    const matchesModality = modalityFilter === 'Todas' || p.level === modalityFilter;
          ...
          <div className="flex flex-wrap gap-4 items-center justify-between">
            ...
            <div className="flex bg-gray-100 rounded-lg p-1">
              <button
                ...
                className={`p-2 rounded-md transition-colors ${viewMode === 'grid' ? 'bg-white shadow-sm text-red-600' : 'text-gray-600'
                  }`}
              >
              ...
              <button
                ...
                className={`p-2 rounded-md transition-colors ${viewMode === 'list' ? 'bg-white shadow-sm text-red-600' : 'text-gray-600'
                  }`}
              >
                ...
                >
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <button
                      ...
                      className={`p-1 rounded-full transition-colors ${participant.favorite ? 'text-yellow-500' : 'text-gray-400 hover:text-yellow-500'
                        }`}
                    >
              ...
              <div className="flex items-center gap-6">
                ...
                <div className="flex gap-3">
                  <button
                    ...
                    className={`px-4 py-2 rounded-lg font-medium transition-colors ${isTimerRunning
          ...
          <div className="bg-white rounded-xl shadow-lg p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {currentRoundData.map((participant, index) => (
                <div
                  key={participant.id}
                  className={`p-4 rounded-lg border-2 transition-all ${index === currentPairIndex
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm ${index === currentPairIndex
              ...
              <table className="w-full">
                ...
                <tbody>
                  {rankings.map((participant, index) => (
                    <tr key={participant.id} className={`border-b border-gray-100 hover:bg-gray-50 ${index < 3 ? 'bg-gradient-to-r from-yellow-50 to-orange-50' : ''
                      }`}>
                      <td className="py-4 px-4">
                        <div className="flex items-center gap-2">
                          <span className={`font-bold text-lg ${index === 0 ? 'text-yellow-600' :
  ...
  return (
    <div className="relative">
      ...
      <div className="fixed bottom-6 right-6 bg-white rounded-full shadow-lg p-2 flex gap-2">
        <button
          onClick={() => setCurrentScreen('pre-dance')}
          className={`p-3 rounded-full transition-colors ${currentScreen === 'pre-dance' ? 'bg-red-600 text-white' : 'text-gray-600 hover:bg-gray-100'
            }`}
          title="Pre-baile"
        >
        ...
        <button
          onClick={() => setCurrentScreen('rounds')}
          className={`p-3 rounded-full transition-colors ${currentScreen === 'rounds' ? 'bg-red-600 text-white' : 'text-gray-600 hover:bg-gray-100'
            }`}
          title="Tandas"
        ...
        <button
          ...
          className={`p-3 rounded-full transition-colors ${currentScreen === 'results' ? 'bg-red-600 text-white' : 'text-gray-600 hover:bg-gray-100'
            }`}
          title="Resultados"
        ...
        <button
          ...
          className={`p-3 rounded-full transition-colors ${currentScreen === 'config' ? 'bg-red-600 text-white' : 'text-gray-600 hover:bg-gray-100'
            }`}
          title="Configuración"
        >
